...
### Waveshare ESP32-S3 Touch LCD 1.46/1.46C 扬声器输出增强器保守版

#### 背景

Waveshare ESP32-S3 Touch LCD 1.46/1.46C 的外放音量和听感需要在不影响语音输入链路的前提下增强。本轮先上传保守版参数，只在播放端加入 `SpeakerOutputEnhancer`，让云端 TTS、电话类下行音频以及本地 `PlaySound()` OGG 提示音统一经过同一套后处理。
...
- 实机听感：当前保守版参数未发现破音，可作为第一版上传到 GitHub；后续仍可继续微调外放响度、失真感和提示音/TTS 平衡。
